...
gulp.task(
    "build",
    gulp.series(clean, gulp.parallel(media, "scripts", styles, other), html)
);
gulp.task(clean);
gulp.task(format);
gulp.task(watch);
...
function format() {
    return gulp
        .src(paths.styles, { base: __dirname })
        .pipe(csscomb())
        .pipe(gulp.dest("."));
}
...
function media() {
    return gulp
        .src(paths.media, { since: gulp.lastRun(media) })
        .pipe(imagemin())
...
function bundleScripts() {
    return browserify(glob.sync("tmp/**/[^_]*.js"), { debug: true })
        .bundle()
